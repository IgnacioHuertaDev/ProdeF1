...
import {
  getAllPilotosResults,
  getCantidadAciertosAbandono,
  getCarreraApiByPrediction,
  getMultiplicadorAbandonos,
...
function calculatePredictions(
  ...
  const raceTotalLaps = Number(
    carrera.Results?.find((x) => x.position == '1')?.laps ?? 0
  )
  ...
  const aciertosAbandonos = getCantidadAciertosAbandono(
    predictions,
    pilotosAbandonos
  )
  const puntosAbandonosLog = ReglaPuntajeAbandono.get(aciertosAbandonos) ?? 0
  const multiplicadorAbandono = getMultiplicadorAbandonos(
    predictions,
    pilotosAbandonos,
    raceTotalLaps
  )
  penalidadAbandonos = getPenalidadAbandonos(
    predictions,
    pilotosAbandonos,
    allPilots
  )
  ...
  logResults(
    puntosAciertos,
    puntosAciertosOrden,
    puntosAciertosMasOrden,
    puntosAbandonosLog,
    multiplicadorAbandono,
    penalidadAbandonos,
    total,
    raceTotalLaps,
    pilotosAbandonos
...
const logResults = (
  aciertos: number,
  aciertosOrden: number,
  aciertosMasOrden: number,
  abandonos: number,
  multiplicadorAbandono: number,
  penalidadAbandonos: number,
  total: number,
